...
use midenid_contracts::{notes::{create_price_set_note, create_pricing_initialize_note}, utils::{create_account, create_naming_account, create_naming_library, create_pricing_account}};
...
async fn test_pricing_set_price() -> anyhow::Result<()> {
    ...
    assert!(result.is_ok(), "price_set should validated successfully: {:?}", result.err());
