...
async fn test_initialize_registry() -> anyhow::Result<()> {
    ...
    assert_eq!(init_word.get(0).unwrap().as_int(), 1, "Registry should be initialized");
    ...
    assert_eq!(token_prefix, ctx.fungible_asset.faucet_id().prefix().as_u64());
    assert_eq!(token_suffix, ctx.fungible_asset.faucet_id().suffix().as_int());

    // Verify price is set (slot 5)
    let price_slot = ctx.naming.storage().get_item(5)?;
    let price_word: Word = price_slot.into();
    assert_eq!(price_word.get(0).unwrap().as_int(), 100, "Price should be 100");
